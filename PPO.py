...
class ActorNetwork(nn.Module):
    def __init__(self, n_actions, input_dims, alpha,
        ...
        # loss of actor will be negative, because we are doing gradient ascent

        self.checkpoint_file =" PPO_actor.pth"
...
class CriticNetwork(nn.Module):
    def __init__(self, input_dims, alpha, fc1_dims=64, fc2_dims=64,
        ...
        self.checkpoint_file = "PPO_critic.pth"
...
class Agent:
    def __init__(self, n_actions, input_dims, gamma=0.99, alpha=1e-3, gae_lambda=0.95,
                 policy_clip=0.2, batch_size=128, n_epochs=20, beta=0.01):
        self.gamma = gamma
        ...
        #self.scheduler_actor = optim.lr_scheduler.ReduceLROnPlateau(self.actor.optimizer, 'min')
        #self.scheduler_critic = optim.lr_scheduler.ReduceLROnPlateau(self.critic.optimizer, 'min')
    ...
    def learn(self):
        for _ in range(self.n_epochs):
            ...
            for batch in batches:
                ...
                actor_loss = -T.min(weighted_probs, weighted_clipped_probs).mean() #- self.beta * entropy)
...
def train_agent():
    ...
    for i in range(n_games):
        ...
        print('episode', i+1, 'score %.1f' % score, 'avg score %.1f' % avg_score,
                'time_steps', n_steps, 'learning_steps', learn_iters, "learning_rate", alpha) 
    x = [i+1 for i in range(len(score_history))]
    T.save(agent.actor.state_dict(), 'ppo_params.pth')
